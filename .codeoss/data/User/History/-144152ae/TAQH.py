...
def load_data() -> pd.DataFrame:
    ...
    df = df.dropna(subset=["decile_score", "race"])

    # Convert numeric columns from strings to proper types
    numeric_cols = [
        "age", "decile_score", "decile_score_2", "v_decile_score",
        "juv_fel_count", "juv_misd_count", "juv_other_count",
        "priors_count", "priors_count_2",
        "two_year_recid", "is_recid", "is_violent_recid",
        "days_b_screening_arrest", "c_days_from_compas",
        "r_days_from_arrest", "start", "end", "event"
    ]
    for col in numeric_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    # Fill NaN only in columns that exist
