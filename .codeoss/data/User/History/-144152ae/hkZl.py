...
def explain_individual(person_id: int):
    ...
    try:
        ...
        explainer   = shap.TreeExplainer(model)
        shap_values = explainer.shap_values(x_individual)[0]   # shape (n_features,)
        ...
        for feat in features:
            ...
            if feat in encoders:
                ...
                decoded[feat] = round(float(val), 2) if val is not None else None
    ...
    try:
        ...
        for i in range(len(df_raw)):
            ...
            results.append({
                ...
                "risk_prob":  round(float(probs[i]), 3),
                "risk_label": "High risk" if preds[i] == 1 else "Low risk",
            })
